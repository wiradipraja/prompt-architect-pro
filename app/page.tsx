...
// Di masa depan, kita akan membuat komponen kustom untuk ini
...
export default function ArchitectPro() {
  ...
  const handleGenerate = () => {
    // Merakit prompt akhir dari semua state
    const finalPrompt = 
    `${characterDetails}, ${subject}, dengan gaya ${style}, ` +
    `dilihat dari sudut pandang ${angle}. ` +
    `--no ${negativePrompt}`;

    const finalVideoPrompt = 
    `VIDEO: ${finalPrompt} | DIALOG: "${dialogue}"`;

    console.log("--- PROMPT TEXT-TO-IMAGE ---");
    ...
    // Di fase berikutnya, di sini kita akan memanggil API terjemahan dan menyimpan ke Firestore
  };
      ...
      <div className="w-full max-w-4xl bg-gray-800 p-8 rounded-xl shadow-2xl shadow-blue-500/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-4">
            <div>
              ...
              <textarea
                ...
                className="w-full h-24 bg-gray-700 border border-gray-600 rounded-md p-3 text-white focus:ring-2 focus:ring-blue-500"
                placeholder="Apa inti dari adegan ini?"
              />
            </div>
            <div>
              ...
              <textarea
                ...
                className="w-full h-32 bg-gray-700 border border-gray-600 rounded-md p-3 text-white focus:ring-2 focus:ring-blue-500"
                placeholder="Jelaskan karakter secara detail atau pilih dari pustaka..."
              />
            </div>
             <div>
              ...
              <textarea
                ...
                className="w-full h-20 bg-gray-700 border border-gray-600 rounded-md p-3 text-white focus:ring-2 focus:ring-blue-500"
                placeholder="Tuliskan dialog yang akan diucapkan..."
              />
            </div>
          </div>
          ...
          <div>
            ...
            <SteppedSlider 
              ...
              value={style}
              onChange={setStyle}
              options={["Fotorealistis", "Sinematik", "Gaya Anime", "Lukisan Minyak", "Surealis"]}
              explanation="Pilih nuansa artistik dan emosional dari output."
            />
            <SteppedSlider 
              label="Sudut Kamera (Angle)"
              value={angle}
              onChange={setAngle}
              options={["High Angle", "Eye-Level", "Low Angle", "Dutch Angle", "Top-down View"]}
              explanation="Pilih sudut pandang kamera untuk menentukan perspektif."
            />
             <div>
              ...
              <input
                ...
                className="w-full bg-gray-700 border border-gray-600 rounded-md p-2 text-white focus:ring-blue-500 focus:border-blue-500"
                placeholder="contoh: buram, teks, watermark"
              />
            </div>
          </div>
        </div>
        ...
        <div className="mt-10 text-center">
            <button
                ...
                className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-10 rounded-lg text-lg transition-transform transform hover:scale-105"
            >
                Buat Prompt
            </button>
        </div>
